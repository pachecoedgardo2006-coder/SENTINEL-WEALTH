...
import { getPortfolioSummary } from '../api/sentinel';
...
const HnwDashboardView = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const json = await getPortfolioSummary();
        setData(json);
      } catch (err) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            {data?.clientName ? `${data.clientName} · ${data.domicile ?? ''}` : 'Monitoreo activo de patrimonio y prevención de amenazas'}
          </p>
        </div>
